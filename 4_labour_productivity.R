...
hc.calculator <- function(region){
  ...
  dt.wage[ , wage := (wagetemp$average_scenario1)*12*wage_adjuster] 
  
  ### Excess Life Years Lost & Lost Wages from death #######

  r <- 0.03 ## 3% discount rate 
  ### !!! future iterations move out of the function into an input into the function
  ...
  prod[ , z_x := bigl_x*wage_age] ## costed person years
  
  for (i in 1:nrow(prod)){
    prod[i , t_adj := sum(prod$z_x[i:nrow(prod)])]
  }
  
  prod[ , prod_x := t_adj/l_person] ## costed person years / person years
  ...
  hc.prod[ , averted_HC := vaccine_avertable_deaths_resistant*dprod_hc]
  hc.prod[ , HC_cost := deaths_resistant*dprod_hc]
...
deaths.prod.global <- data.table::melt(deaths.prod, id.vars = c("Pathogen" ,
                       ...
                       "vaccine-target disease" ,
                         ...
                         "duration"         ,
                            "target disease" ,       
                        "target population"), 
                       measure.vars = list(c('mean_total_AFRO' ,
                                             'mean_total_EMRO',
                                             'mean_total_EURO' ,
                                             'mean_total_AMRO',
                                             'mean_total_SEARO',
...
hc.calculator.WLYL <- function(region){
  ...
  r <- 0 ## put to 0 for the working life years lost debate on what to set to ??
  ### !!! future iterations move out of the function into an input into the function
  ...
  prod[ , z_x := bigl_x*1] ####!! For simplicity just changing this (dont technically need dt.wage for this one)
...
output_lst <- list()
unique(cases$who.region)
output_lst$WPRO <- wlyl_summary(hc.calculator.WLYL(region="WPRO"))
output_lst$SEARO <- wlyl_summary(hc.calculator.WLYL(region="SEARO"))
output_lst$EURO <- wlyl_summary(hc.calculator.WLYL(region="EURO"))
output_lst$PAHO <- wlyl_summary(hc.calculator.WLYL(region="PAHO"))
output_lst$AFRO <- wlyl_summary(hc.calculator.WLYL(region="AFRO"))
output_lst$EMRO <-wlyl_summary(hc.calculator.WLYL(region="EMRO"))
output_hc <- rbindlist(output_lst, idcol=TRUE)
...
# # #### doing for each region
hc.calculator.sc2 <- function(region){
  ...
  wagetemp <- wage_region[who.region==country]
  dt.wage <- wage_template
  dt.wage[ , wage := (wagetemp$average_scenario2)*12*wage_adjuster] ##!!! heres the difference
