...
across_vaccines <- function(cases_averted){
  ...
  combo_chunks <- function(DRI.all, cases_averted,
    ...
    ## get values per run so can feed back in 
    #### !!! perhaps next iteration try differnet groupings (e.g WHOChoice or income groups)
    #### !!! also as weights based on whole data frame, currently skews towards los values where
    #### there are more exposure/vaccine groups/age groups affected
    #### next iteration might want to find away to separate --> calculate --> merge back in to stop that from happening
    
    ### class + gram stain
    ### !! check if Salmonella matched correctly
    ...
    av.los.region.class.gram <- cost_cases_c_nonweight %>% group_by(WHO.Region,class,gram.stain,
                                                                    run)%>%
      summarise(av_los_class_gram := mean(los.DRI))%>%
      as.data.table
    
    ### class different/any gram stain
    av.los.region.class <- cost_cases_c_nonweight %>% group_by(WHO.Region,class,
                                                               run)%>%
    ...
    ## e.g. Gastro
    av.los.region.syndrome <- cost_cases_c_nonweight %>% group_by(WHO.Region, 
                                                                  syndrome,
                                                                  run)%>%
      summarise(av_los_syndrome := mean(los.DRI))
    
    ## do a final by region 
    av.los.region <- cost_cases_c_nonweight %>% group_by(WHO.Region,
                                                         run)%>%
    ...
    no_cost_cases_class_gram <- merge(no_cost_cases, av.los.region.class.gram,
                                      by=c("WHO.Region", "class","gram.stain"),
    ...
    no_cost_cases_class <- merge(no_cost_cases_class_gram_empty, av.los.region.class,by=c("WHO.Region", "class"),
                                 all.x=TRUE, all.y=FALSE, allow.cartesian=TRUE)
    ...
    all_costed <- rbindlist(all_costed, fill=TRUE)
    ...
    ### put in whoc for those who used averages
    all_costed_whoc <- all_costed[!is.na(whoc.region)]
    all_costed <- all_costed[is.na(whoc.region)]
    
    who_whoc_wb <- as.data.table(who_whoc_wb)
    who_whoc_wb <- who_whoc_wb[ , c("iso3c","whoc.region")]
    
    ### combining
    all_costed <- merge(all_costed, who_whoc_wb, by = c("iso3c"),
                        all.x=TRUE, all.y=FALSE,
                        allow.cartesian=TRUE)
    all_costed[is.na(whoc.region.x),whoc.region.x:=whoc.region.y]
    
    setnames(all_costed,"whoc.region.x","whoc.region",skip_absent=TRUE)
    all_costed <- all_costed[ , -c("whoc.region.y")]
    
    all_costed <- rbind(all_costed,all_costed_whoc)
    ### !!! note there are still some iso3c codes who do not have a mapped whoc
    ### will have to use WHO.Region weighted averages for these later on 
    ### if no individual country whoc estimates either - think for now ok
    rm(all_costed_whoc)
    gc()
    ...
    ## "PRK" "SOM" "SSD" "ZWE"
    ## SSD was not in list so manually edit for now
    DRI.whoc.n[iso3c=="SSD", whoc.region := "AFRO E"]
...
save(vaccine_output_dt, 
     file=paste0("C:/Users/nichola.naylor/Documents/WHO_2023/VAF_globalburden/outputs/fulloutput_chunks/results",i,".RData"))
