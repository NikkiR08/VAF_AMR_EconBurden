...
f5_1 <- ggplot(hospital_avert, aes(x = Pathogen, y = avert_cost_med)) +
  ...
  facet_wrap(~.id)+ 
  ggtitle("(A) Hospital Cost Burden") +
...
f5_2 <- all.dt %>%
  ...
  labs(
    title = "(B) Averted Labour Productivity Losses ($)",
    y = "Pathogen"
  ) +
  theme_minimal() +
  theme(
...
f5_3 <- all.dt2 %>%
  ...
  labs(
    title = "(C) Averted Working-Life-Years-Lost",
    y = "Pathogen"
  ) +
  theme_minimal() +
  theme(
...
tiff("Figure5_new.tiff", units="in", width=19, height=16, res=300)

# Arrange with widths (2:1 ratio) and p2 at half height
combined_plot <- free(f5_1)/f5_2/f5_3  + 
...
################################ THERE MAY BE DUPLICATION BUT RESTARTING FROM HERE ##############################

############## CREATING PLOTS FROM OUTPUTS _ 2 ##############
...
### this is used for top line narrative results in paper
...
# 
tiff("Figure4.tiff", units="in", width=17.5, height=7, res=300)
...
UNITcost_averted_global[class=="Fluoroquinolones or MDR in Salmonella" ,
                         class:="fluoroquinolones/MDR" ]

UNITcost_averted_global[ ,Exposure_Group := paste(Pathogen,";",class)]
...
ggplot(UNITcost_averted_global2, aes(x=interaction(Exposure_Group), y=Median_unitcost,
  ...
  xlab("Exposure group (bacteria;ABR)")+
  ggtitle("Syndrome")
...
TB[Exposure_Group=="Mycobacterium tuberculosis ; mdr", Exposure_Group:="MDR TB"]
