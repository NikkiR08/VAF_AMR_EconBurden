...
#### !!! differences for testing #####
  ...
  cases_averted <- cases_averted_all[vaccine_id=="E. coli - non-diarrhogenic_0.7_0.7_5 years_All except Diarrhoea_All age groups"   |
                                                                      vaccine_id=="Staphylococcus aureus_0.6_0.7_5 years_All_All age groups"|
                                                                      vaccine_id== "ETEC_0.6_0.7_5 years_Diarrhoea_6 months" |
                                       vaccine_id=="Salmonella Typhi_0.85_0.7_15 years_All_9 months"]

DRI.all <- DRI.all[run<=100]

  cases_list = split(cases_averted, by=c("vaccine_id")) 
  ...
  save(vaccine_output_dt, file="outputs/testing_hospitalfunctions.RData")## e. coli, staph and salmonella


  # load("outputs/testing_hospitalfunctions.RData")
  
  ##### getting outputs from vaccine_output_dt
  ### want Region, cases, average LOS, average LOS cost, average days_cases averted & costs averted
test <- vaccine_output_dt
  
  test_vaccineoutput <-  vaccine_output_dt %>% group_by(WHO.Region, Pathogen, 
                                                        Infectious.syndrome,
                                                        vaccine_id, run) %>%
    summarise(cases_averted_hospitalised = sum(cases_averted_hospitalised, na.rm=TRUE),
              los.DRI = median(los.DRI, na.rm=TRUE),
              ...
              avertable_cost_cases = sum(avertable_cost_cases, na.rm=TRUE)) %>%
    group_by(WHO.Region,
             Pathogen, Infectious.syndrome,
             vaccine_id) %>%
    summarise(cases_averted_hospitalised = median(cases_averted_hospitalised, na.rm=TRUE),
              los.DRI = median(los.DRI, na.rm=TRUE),
              los.cost = median(los.cost, na.rm=TRUE),
              avertable_days_cases = median(avertable_days_cases, na.rm=TRUE),
              avertable_cost_cases = median(avertable_cost_cases, na.rm=TRUE))
  
  
  test_nosyndrome <-  vaccine_output_dt %>% group_by(WHO.Region, vaccine_id, run) %>%
    summarise(cases_averted_hospitalised = sum(cases_averted_hospitalised, na.rm=TRUE),
              los.DRI = median(los.DRI, na.rm=TRUE),
              los.cost = median(los.cost, na.rm=TRUE),
              avertable_days_cases = sum(avertable_days_cases, na.rm=TRUE),
  ...
  ##### comparing to full data sets
  load("outputs/fulloutput_chunks/Stap17.RData")
  staph <- vaccine_output_dt
  
  load("outputs/fulloutput_chunks/E. c2.RData")
  expec <- vaccine_output_dt
  load("outputs/fulloutput_chunks/ETEC3.RData")
  etec <- vaccine_output_dt
  
  load("outputs/fulloutput_chunks/Salm23.RData")
  salm <- vaccine_output_dt
  rm(vaccine_output_dt)
  
  
  list.outputs <- list(staph, expec, etec, salm)
  rm(salm)
  rm(staph)
  rm(expec)
  rm(etec)
  
  outputs_full <- rbindlist(list.outputs)
  
  test_vaccineoutput_full <-    outputs_full %>% group_by(WHO.Region, Pathogen, 
                                                        Infectious.syndrome,
                                                        vaccine_id, run) %>%
    summarise(cases_averted_hospitalised = sum(cases_averted_hospitalised, na.rm=TRUE),
              los.DRI = median(los.DRI, na.rm=TRUE),
  ...
  test_nosyndrome_full <-   outputs_full %>% group_by(WHO.Region, vaccine_id, run) %>%
    summarise(cases_averted_hospitalised = sum(cases_averted_hospitalised, na.rm=TRUE),
              los.DRI = median(los.DRI, na.rm=TRUE),
              los.cost = median(los.cost, na.rm=TRUE),
              avertable_days_cases = sum(avertable_days_cases, na.rm=TRUE),
