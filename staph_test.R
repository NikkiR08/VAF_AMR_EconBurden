...
cases_averted_all <- cases_averted
  ...
  vaccines1 <- "Staphylococcus aureus_0.6_0.7_5 years_All_6 weeks & elderly age group"   
  ...
  ## #earlier on - leave for now as running
    ...
    combo_chunks <- function(DRI.all, cases_averted,
      ...
      cases=sum(cases_resistant,na.rm=TRUE))
    ...
    sdf <- list(sdf[[1]],sdf[[2]]) ## !! for quick test just running 2 times
  ...
  #### test for Staph as previously bed days greater in AFRO when just
  ### had 1 vaccine scenario given
  load("outputs/fulloutput_chunks/resultsStap16.RData")
  
  
  test.bd <- vaccine_output_dt%>% group_by(WHO.Region, Pathogen, 
                                           vaccine_id, Infectious.syndrome, run) %>%
    summarise(cases_hospitalised = sum(cases_hospitalised, na.rm=TRUE),
              los.DRI = sum(los.DRI, na.rm=TRUE),
              days_cases = sum(days_cases, na.rm=TRUE)) %>%
    group_by(WHO.Region,
             Pathogen, 
             vaccine_id,Infectious.syndrome) %>%
    summarise( cases_hospitalised = median(cases_hospitalised, na.rm=TRUE),
               los.DRI = median(los.DRI, na.rm=TRUE),
               days_cases = median(days_cases, na.rm=TRUE))
  
  load("data_inputs/epi_inputs_all.RData")
  
  staph <- all_data[Pathogen=="Staphylococcus aureus"]
  
  test.bd2 <- staph %>% group_by(WHO.Region, Pathogen, 
                                 vaccine, 
                                 Efficacy,Coverage, Duration,target_population,
                                 Infectious.syndrome) %>%
    summarise(cases = sum(cases_resistant, na.rm=TRUE)) %>%
    as.data.table()
  
  
  
  ### think a lot of it is driven by SSTI proportions  , perhaps wasn't properly
  ### integrated in previous iteration but is now
  ### also potentially a rounding issue - 
  ### "result" from the test above showed highest cases hospitalised as "15116152.7" where as
  ### vaccine_output_dt from the loaded .RData read in shows 15116153 that's an adidtion .3 of a person
  ### for each 1000 run for that drug-bug-combo for each country
  ### however the costs are calculated before and then added
  ### although could also be subject to rounding issues prior during multiple summations
  ### and when looking at other countries there is a higher dp for cases_hospitalised...
  ### worth a check in future iterations !!!
  
  ## using variations of this to check specific exposure group
  # skin <-  cost_cases_c[Infectious.syndrome=="Bacterial skin infections" &
  #                         iso3c=="AFG" & Age.group=="Early Neonatal"]
  # 
  ...
  DRI.all <- sdf[[1]]
    ...
    skin <-  DRI.whoc.all[Infectious.syndrome=="Bacterial skin infections" &
                          iso3c=="AFG" & Age.group=="Early Neonatal"]
    
    
    
    ####### full try with new cost sample ############
   ...
   teststaph_new <-  result %>% group_by(WHO.Region, Pathogen, 
                                  vaccine_id, Infectious.syndrome, run) %>%
      summarise(cases_hospitalised = sum(cases_hospitalised, na.rm=TRUE),
                los.DRI = sum(los.DRI, na.rm=TRUE),
                days_cases = sum(days_cases, na.rm=TRUE)) %>%
